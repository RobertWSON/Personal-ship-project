...
class FindClassic extends React.Component   {
    ...
    componentDidMount() {
        //  The if statement below checks to see if the Link path has been picked up when 
        // FindClassic component mounts. In it's initial state, it is the inital finding of the Link.  
        if (this.props.location.state) {
            this.setState({ classicship: this.props.location.state})
            //This else part matches the state property in the Link with classic_ship_name 
            // and this is what helps display a ship page.  
        }   else    {
            this.setUpClassic(this.props.match.params.classic_ship_name)
        }
    }

    // This function is for the set up of the classic ship and it has been given a temporary name 
    // for the parameter (ClassicName). 
    ...
    render()    {
        ...
        // We do not need to map for a ship because we have already picked up the ship by a key id 
        // in Evolution Component for Evolution Part 1 or Evolution Part 2. 
        // We now have the correct ship, so we can Create our own loading component.

        // Trying encoding below, but does not work here
